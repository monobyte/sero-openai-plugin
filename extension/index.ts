...
import type { Column, KanbanState, Priority } from '../shared/types';
...
/**
 * Actions that read-modify-write state.json; they are dispatched under the
 * shared `<stateFile>.lock` mutex (#428). `cleanup` and `cancel-pr` are
 * deliberately absent: they run slow git/network work and take the lock
 * themselves around only their final state write.
 */
const LOCKED_ACTIONS = new Set<string>([
  'add', 'move', 'update', 'delete', 'start', 'approve', 'complete',
  'retry', 'settings', 'request-revisions',
]);
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        const dispatch = async (state: KanbanState): Promise<AgentToolResult<unknown>> => {
        ...
        if (LOCKED_ACTIONS.has(params.action)) {
          // Hold the shared lock across read + write so a tool call cannot
          // interleave with the Sero host writing the same file for the UI.
          return await withStateLock(statePath, async () => dispatch(await readState(statePath)));
        }
        return await dispatch(await readState(statePath));
      } catch (error) {
