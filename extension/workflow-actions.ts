 ...
 *
 * Handlers that mutate state.json are dispatched under the shared
 * `<stateFile>.lock` mutex by index.ts (#428). `handleCleanup` is the
 * exception: it runs slow git work unlocked and takes the lock itself
 * around its final re-read + write.
 */
...
export async function handleCleanup(
  ...
  // The git cleanup above is slow, so it ran without the shared state lock;
  // re-read and merge under `<stateFile>.lock` instead of writing the stale
  // snapshot (#428).
  await withStateLock(statePath, async () => {
    const current = await readState(statePath);
    for (const card of current.cards) {
      if (cleanedIds.has(card.id)) card.worktreePath = undefined;
    }
