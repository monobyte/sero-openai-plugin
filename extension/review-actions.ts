...
export async function handleCancelPR(
  ...
  // The PR close and worktree cleanup above are slow, so they ran without the
  // shared state lock; re-read and merge under `<stateFile>.lock` instead of
  // writing the stale snapshot (#428).
  await withStateLock(statePath, async () => {
    const current = await readState(statePath);
    const target = current.cards.find((c) => c.id === id);
    if (!target) return;
    target.column = 'backlog';
    target.status = 'idle';
    target.error = '[PR CANCELLED] PR was cancelled by user and card returned to backlog.';
    target.prUrl = undefined;
    target.prNumber = undefined;
    target.branch = undefined;
    target.worktreePath = undefined;
    target.previewServerId = undefined;
    target.previewUrl = undefined;
    target.reviewFilePath = undefined;
    target.planningProgress = undefined;
    target.implementationProgress = undefined;
    target.reviewProgress = undefined;
